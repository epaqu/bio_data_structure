...
void printpoly(int term, int **poly)
{
	if (firstOrSecond == 1)
	...
	else
	{
		...
		{
			if (poly[0][i] != 1 && poly[0][i] != 0)
			{
				if (poly[1][i] != 1 && poly[1][i] != 0)
					cout << poly[0][i] << "x^" << poly[1][i] << " + ";
				else if (poly[1][i] == 1)
					cout << poly[0][i] << "x" << " + ";
				else if (poly[1][i] == 0)
					cout << poly[0][i] << " + ";
		...
		if (poly[0][term-2] != 1)
		{
			if (poly[1][term-2] != 1 && poly[1][term-2] != 0)
				cout << poly[0][term-2] << "x^" << poly[1][term-2];
			else if (poly[1][term-2] == 1)
				cout << poly[0][term-2] << "x";
			else if (poly[1][term-2] == 0)
				cout << poly[0][term-2];
		}
		else
			if (poly[1][term-2] != 1 && poly[1][term-2] != 0)
				cout << "x^" << poly[1][term-2];
			else if (poly[1][term-2] == 1)
				cout << "x";
			else
				cout << poly[0][term-2];
...
int **sortDecreasing(int **poly, int size)
{
	int largest, id;
...
int main()
{
	int **poly1 = getpoly();
	int size1 = size;
	printpoly(size1, sortDecreasing(poly1, size1-1));
	firstOrSecond++;
	int **poly2 = getpoly();
	int size2 = size;
	printpoly(size2, sortDecreasing(poly2, size2-1));
	...
	for (int i = 0; i < size1-1; i++)
	{
		...
		{
			multiplied[0][index] = poly1[0][i] * poly2[0][j];
			multiplied[1][index] = poly1[1][i] + poly2[1][j];
			index++;
		}
	}
	//done multiplying
